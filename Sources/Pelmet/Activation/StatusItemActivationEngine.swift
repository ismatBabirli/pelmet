...
protocol StatusItemActivating: AnyObject {
    ...
    /// Descriptors the Shelf deriver uses to offer pointer-safe one-click
    /// activation — empty unless Accessibility access is available. A target
    /// can still decline activation if it advertises no supported action.
...
final class StatusItemActivationEngine: StatusItemActivating {
    ...
    /// Live AX elements for the current directory's pointer-free action path.
    ...
    private func rebuildDirectory() {
        ...
        let canReadAccessibility = canActivate
